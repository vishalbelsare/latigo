...
def test_model_info():
    ...
    for i in range(num):
        ...
        spec = model.get_spec()
        logger.info(
            f"PROJECT:{model.project_name} MODEL:{model.model_name}({i}) TAG_LIST: {pprint.pformat(model.tag_list)} TARGET_TAG_LIST: {pprint.pformat(model.target_tag_list)} SPEC:{spec}"
        )
...
def un_test_prediction_execution():
    ...
    logger.info("CONFIG:")
    logger.info(pprint.pformat(config))
    gordo_prediction_execution_provider = GordoPredictionExecutionProvider(
        dummy_provider, dummy_prediction_forwarder, config
    )
    project_name: str = config.get("projects", ["no-projects-in-config"])[0]
    model_name: str = "lol"
    logger.info(
        "Prediction for project_name={project_name} and model_name={model_name}"
    )
    ...
    prediction_data = gordo_prediction_execution_provider.execute_prediction(
        project_name=project_name, model_name=model_name, sensor_data=sensor_data
    )

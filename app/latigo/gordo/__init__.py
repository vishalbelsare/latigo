...
from latigo.types import TimeRange, SensorDataSpec, SensorDataSet, PredictionDataSet, LatigoSensorTag
...
def _gordo_to_latigo_tag_list(gordo_tag_list: typing.List[SensorTag]) -> typing.List[LatigoSensorTag]:
...
class PredictionForwarder:
    def __call__(self, *, predictions: pd.DataFrame = None, machine: Machine = None, metadata: dict = dict(), resampled_sensor_data: pd.DataFrame = None) -> typing.Awaitable[None]:
        ...
...
def gordo_config_hash(config: dict):
    key = "gordo"
    parts = ["scheme", "host", "port", "project", "target", "gordo_version", "batch_size", "parallelism", "forward_resampled_sensors", "n_retries", "use_parquet"]
    if config:
        for part in parts:
...
def clean_gordo_client_args(raw: dict):
    whitelist = ["project", "target", "host", "port", "scheme", "gordo_version", "metadata", "data_provider", "prediction_forwarder", "batch_size", "parallelism", "forward_resampled_sensors", "n_retries", "session", "use_parquet"]
    args = {}
    for w in whitelist:
        args[w] = raw.get(w)
    return args
...
def _get_machine_tag_list(machine: Machine) -> typing.List[LatigoSensorTag]:
    tag_list_data = machine.dataset.tag_list
    tag_list = []
    for tag_data in tag_list_data:
        tag = LatigoSensorTag(name=tag_data.name, asset=tag_data.asset)
        tag_list.append(tag)
    # logger.info("MODEL 0 META TAG_LIST:" + pprint.pformat(tag_list))
    return tag_list


def _get_machine_target_tag_list(machine: Machine):
    target_tag_list = machine.dataset.target_tag_list
    # logger.info("MODEL 0 META TAG_LIST:"+pprint.pformat(tag_list))
    return target_tag_list


def _get_machine_name(machine: Machine):
    machine_name = machine.name or ""
    # logger.info(f"MODEL NAME: {name}")
    return machine_name


def _get_project_name(machine: Machine):
    project_name = machine.project_name or ""
    # logger.info(f"MODEL NAME: {name}")
    return project_name
...
class GordoModelInfoProvider(ModelInfoProviderInterface):
    ...
    def get_model_data(self, projects: typing.Optional[typing.List] = None, machine_names: typing.Optional[typing.List] = None) -> typing.List[Machine]:
    ...
    def get_all_models(self, projects: typing.List) -> typing.List[Model]:
        machines = self.get_machine_data(projects)
        models = []
        for machine in machines:
            if machine:
                project_name = machine.project_name or "unnamed"
                machine_name = machine.name or "unnamed"
                model = Model(project_name=project_name, model_name=machine_name, tag_list=machine.dataset.tag_list, target_tag_list=machine.dataset.target_tag_list)
                if model:
                    models.append(model)
        return models

    def get_machine_by_key(self, project_name: str, machine_name: str) -> typing.Optional[Model]:
        machines = self.get_machine_data(projects=[project_name], machine_names=[machine_name])
        if not machines:
            return None
        model = None
        machine = machines[0]
    ...
    def get_spec(self, project_name: str, machine_name: str) -> typing.Optional[SensorDataSpec]:
        model = self.get_machine_by_key(project_name=project_name, machine_name=machine_name)
        if not model:
            return None
...
def print_client_debug(client: typing.Optional[Client]):
    ...
    if not client:
        logger.info("  None")
        return
    data = {"base_url": client.base_url, "metadata": client.metadata, "revision": client.revision, "prediction_forwarder": client.prediction_forwarder, "data_provider": client.data_provider, "use_parquet": client.use_parquet, "session": client.session, "prediction_path": client.prediction_path, "batch_size": client.batch_size, "parallelism": client.parallelism, "forward_resampled_sensors": client.forward_resampled_sensors, "n_retries": client.n_retries, "query": client.query, "target": client.target}
...
class GordoPredictionExecutionProvider(PredictionExecutionProviderInterface):
    ...
    def execute_prediction(self, project_name: str, machine_name: str, sensor_data: SensorDataSet) -> PredictionDataSet:
        if not project_name:
        ...
        if not sensor_data.data:
            logger.warning(f"No data in prediction for project '{project_name}' and model {machine_name}")
            return PredictionDataSet(time_range=sensor_data.time_range, data=None, meta_data={})
        if len(sensor_data.data) < 1:
            logger.warning(f"Length of data < 1 in prediction for project '{project_name}' and model {machine_name}")
            return PredictionDataSet(time_range=sensor_data.time_range, data=None, meta_data={})
        client = self.gordo_pool.allocate_instance(project_name)
        if not client:
        ...
        return PredictionDataSet(meta_data={project_name: project_name, machine_name: machine_name}, time_range=sensor_data.time_range, data=result)
